...
export function cityBuildTools(
  ...
  return {
    sync(
      ...
      for (let i = placed.length - 1; i >= 0; i--)
        if (
          ...
          dispose(placed[i].object);
          placed.splice(i, 1);
        }
      for (const p of items)
        if (!placed.some((q) => q.id === p.id)) {
          const object = model("sandbags");
    ...
    dispose() {
      clear();
      placed.forEach((p) => dispose(p.object));
      panel.remove();
