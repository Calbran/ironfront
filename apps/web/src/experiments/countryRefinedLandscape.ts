...
import {countryLandscapeSamples,type LandscapePlan} from "../../../../packages/game-core/src/countryLandscape";
export {countryLandscapeSamples,countryMeadowNoise} from "../../../../packages/game-core/src/countryLandscape";
...
export function countryRefinedLandscape(
  ...
  const blades: number[] = [];
  for (let i = 0; i < 5; i++) {
    const angle = i * 2.399, x = Math.cos(angle) * .16, z = Math.sin(angle) * .16;
    const dx = Math.cos(angle) * .055, dz = Math.sin(angle) * .055;
    blades.push(x-dx,0,z-dz, x+dx,0,z+dz, x+.07*Math.sin(angle),.24+(i%3)*.07,z+.05);
  }
  ...
  const canopyCells=new Map<string,{x:number;z:number;count:number}>();
  for(const p of samples.trees){const key=`${Math.floor(p.x/12)}:${Math.floor(p.z/12)}`,cell=canopyCells.get(key)??{x:0,z:0,count:0};cell.x+=p.x;cell.z+=p.z;cell.count++;canopyCells.set(key,cell);}
  const canopies=[...canopyCells.values()].map(c=>({x:c.x/c.count,z:c.z/c.count}));
  const canopyGeometry=new T.IcosahedronGeometry(1,0).scale(6.5,1.25,6.5).translate(0,2.5,0);
  ...
  const bankRocks:P[]=[], reeds:P[]=[], pads:P[]=[];
  for(const river of plan.rivers)for(let i=1;i<river.length;i++){
    const a=river[i-1],b=river[i],length=Math.hypot(b.x-a.x,b.z-a.z),nx=-(b.z-a.z)/length,nz=(b.x-a.x)/length;
    for(let d=4;d<length;d+=9){
      const x=a.x+(b.x-a.x)*d/length,z=a.z+(b.z-a.z)*d/length;
      if(x<12||z<12||x>plan.width-12||z>plan.depth-12||plan.roads.bridges.some(b=>Math.hypot(x-b.x,z-b.y)<b.length/2+15))continue;
      const side=Math.sin(d*7+i)>0?1:-1;
      bankRocks.push({x:x+nx*side*9,z:z+nz*side*9});
      for(let j=0;j<4;j++)reeds.push({x:x+nx*side*(7+j*.35),z:z+nz*side*(7+j*.35)+j*.25});
      if(Math.sin(x*.3+z*.4)>.25)pads.push({x:x+nx*side*4,z:z+nz*side*4});
    }
  }
  batch(bankRocks,rock,rockMat,.24,.3,true);
  batch(reeds,grass,grassMat,1.6,0,true);
  const lilyGeometry=new T.CircleGeometry(.22,7).rotateX(-Math.PI/2),lilyMaterial=mat(0x597951);
  ownedGeometry.push(lilyGeometry);
  const lilies=new T.InstancedMesh(lilyGeometry,lilyMaterial,pads.length);
  pads.forEach((p,i)=>{pose.position.set(p.x,.06,p.z);pose.rotation.set(0,i*2.399,0);pose.scale.setScalar(1);pose.updateMatrix();lilies.setMatrixAt(i,pose.matrix);});
  lilies.computeBoundingSphere();root.add(lilies);meshes.push(lilies);
  return {
    ...
    update(camera: T.Camera, target: T.Vector3) {
      ...
      for (const entry of groups) {
        const tileDistance=Math.hypot(entry.center.x-target.x,entry.center.z-target.z);
        const detailed=distance<650&&tileDistance<Math.max(220,distance*.8+100);
        entry.group.visible=entry.tier==="far"?!detailed
          :entry.tier==="tree"?detailed
