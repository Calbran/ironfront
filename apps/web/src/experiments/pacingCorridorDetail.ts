...
export function createCorridorDetail(scene:T.Scene,world:World,rural:ReturnType<typeof generatePacingCountryside>,corridor:PacingCorridor,kit:MiniatureKit,point:(x:number,y:number,h?:number)=>T.Vector3,worldPoint:(p:T.Vector3)=>{x:number;y:number},network?:CountryRoadNetwork,height:(p:T.Vector3)=>number=()=>0){
 const paths=network?.roads.map(r=>r.path)??(corridor?[corridor.path]:[]),roads=paths.flatMap(path=>path.slice(1).map((p,i)=>({a:point(path[i].x,path[i].y),b:point(p.x,p.y)}))),fields=rural.farms.flatMap(f=>f.fields.map(f=>({...f,p:point(f.x,f.y)}))),pois=rural.pois.map(p=>({...p,p:point(p.x,p.y)}));
 ...
 const distance=(p:T.Vector3,segments:typeof roads)=>{let min=Infinity;for(const {a,b} of segments){const dx=b.x-a.x,dz=b.z-a.z,t=T.MathUtils.clamp(((p.x-a.x)*dx+(p.z-a.z)*dz)/(dx*dx+dz*dz||1),0,1);min=Math.min(min,Math.hypot(p.x-a.x-t*dx,p.z-a.z-t*dz));}return min;};
 ...
 function create(x:number,z:number,key:string){
  ...
  const nearby=roads.filter(({a,b})=>Math.max(a.x,b.x)>x*96-100&&Math.min(a.x,b.x)<(x+1)*96+100&&Math.max(a.z,b.z)>z*96-100&&Math.min(a.z,b.z)<(z+1)*96+100);
  for(let i=0;i<350;i++){
   const p=new T.Vector3((x+rand())*96,.12,(z+rand())*96),d=distance(p,nearby);if(d<4||d>100)continue;
   ...
   p.y=height(p)+.12;pose.position.copy(p);pose.rotation.set(0,rand()*Math.PI*2,0);pose.scale.set(s,type===0?s*.6:s,s);pose.updateMatrix();lists[type].push(pose.matrix.clone());
  }
