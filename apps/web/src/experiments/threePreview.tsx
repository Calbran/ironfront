...
function Study() {
  ...
  useEffect(()=>{if(!data||engine!=='three'||!host.current)return;try{const next=miniatureScene(host.current,data,setSelected,setStats,setError,setArmies);scene.current=next;next.configure(settings);next.chooseCity(city);next.selectArmies(armies);return()=>{next.dispose();scene.current=undefined;};}catch(e){setError(String(e));}},[data,engine]);
  ...
  return <main><header><div><span className="eyebrow">IRONFRONT / RENDERER EXPERIMENT</span><h1>A world in miniature</h1></div><a href="/">Original game ↗</a></header>
    ...
    <aside><div className="switch" aria-label="Renderer">{['three','pixi'].map(value=><button key={value} aria-pressed={engine===value} onClick={()=>setEngine(value)}>{value==='three'?'Three.js study':'Pixi baseline'}</button>)}</div>
      ...
      <details><summary>What this experiment proves</summary><p>The current world can drive either renderer. Infantry and the jeep reuse existing models; buildings are simple volume studies or retained sprites. Forests use instancing.</p><p>Terrain remains flat beneath scenery. Winter is a visual palette, not a shipped biome. Units use generated roster positions; combat, cover and campaign saves are not connected.</p><p>Performance numbers reflect this browser and view, not a production benchmark.</p></details>
