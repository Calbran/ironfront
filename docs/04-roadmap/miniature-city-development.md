...
Three.js is the production renderer for the campaign and tactical game. `/` consumes authoritative campaign snapshots and commands; `/country-slice.html` and `/city-diorama.html` develop the same Three.js tactical direction. `/three-preview.html` and `/reference-preview.html` remain read-only development studies. Pixi is archived at `/legacy.html` and receives no new gameplay or parity work.
...
| 7. Production readiness | Profile Three.js on a populated campaign with multiple visible cities, moving/fighting units, shadows, smoke, winter, selection and strategy overlays. Exercise low-end devices and long sessions. | Explicit hardware/frame-time, generation-time, memory and interaction budgets; no resource growth on regeneration; reconnect and long-soak checks pass. |
