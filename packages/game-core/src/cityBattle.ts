...
import {CITY_SHELL_DAMAGE,cityShellImpact,CITY_TANK_RELOAD_SECONDS,cityShellFlightSeconds} from "./cityBallistics";
...
export function battleTrial(tactics: ReturnType<typeof createCityTactics>, multipleBattles = false) {
  ...
  if (multipleBattles) {
    ...
    for (const desired of [{x:-110,z:-70},{x:115,z:100}]) {
      let placed = false;
      for (let radius=0; radius<=80 && !placed; radius+=8) {
        for (let a=0; a<16 && !placed; a++) {
          const center={x:desired.x+Math.cos(a*Math.PI/8)*radius,z:desired.z+Math.sin(a*Math.PI/8)*radius};
          if(centers.some(p=>Math.hypot(p.x-center.x,p.z-center.z)<75))continue;
          for(let direction=0;direction<8 && !placed;direction++) {
            const dx=Math.cos(direction*Math.PI/4),dz=Math.sin(direction*Math.PI/4);
            const points=Array.from({length:6},(_,i)=>({x:center.x+dx*(i<3?-9:9)-dz*((i%3)-1)*1.4,z:center.z+dz*(i<3?-9:9)+dx*((i%3)-1)*1.4}));
            if(!points.every(p=>tactics.walkable(p)) || !points.slice(0,3).every((p,i)=>tactics.segmentClear(p,points[i+3])))continue;
            const firstId=200+centers.length*10;
            points.forEach((p,i)=>trial.units.push({...structuredClone(base),...p,id:firstId+i,friendly:i<3,path:[],guide:[],angle:Math.atan2(dx,dz)+(i<3?0:Math.PI)}));
            centers.push(center);placed=true;
          }
        }
      }
    }
...
export function createCityBattle(
  ...
  const buildSight=()=>new FireVisibility(
  ...
  const sandbags:{id:number;x:number;z:number;angle:number}[]=[];let nextSandbag=1;
  function syncSandbags(){tactics.setSandbags(sandbags);sight=buildSight();}
  function buildSandbags(x:number,z:number,angle:number){
    const reason=validateCityBuild({kind:"sandbags",x,z,angle},sandbags.map(p=>({...p,kind:"sandbags" as const})),p=>tactics.walkable(p),p=>tactics.surfaceHeight(p),trial.units.filter(u=>u.health>0));
    if(reason){message=reason;return;}
    if(!trial.units.some(u=>u.friendly&&u.health>0&&Math.hypot(u.x-x,u.z-z)<=40)){message="Build within 40 units of your troops.";return;}
    sandbags.push({id:nextSandbag++,x,z,angle});syncSandbags();message="Sandbags built: infantry cover; tanks can crush them.";
  }
  function removeSandbags(id:number){const i=sandbags.findIndex(p=>p.id===id);if(i>=0){sandbags.splice(i,1);syncSandbags();message="Sandbags removed.";}}
  ...
  const reactions=new Map<number,{anchor:{x:number;z:number};next:number;health:number;target:string|null}>();
  ...
  const emit = (shot: Omit<CityShot, "id">) => {
    ...
    if ([shot.from, shot.to].every(id => { const u=trial.units.find(v=>v.id===id); return u && visibleToFriendly(u); })) disclosedShots.add(event.id);
    shots.push(event);
  ...
  function visibleToFriendly(unit: TrialUnit) {
    ...
    return trial.units.some(observer => observer.friendly && observer.health > 0 &&
      Math.hypot(observer.x-unit.x, observer.z-unit.z) <= citySightRange(observer.kind) &&
      sight.exposure({...squads.get(observer.id)!,x:observer.x,y:observer.z}, {...squads.get(unit.id)!,x:unit.x,y:unit.z}) > 0);
  }
  const contactMemory=createContactMemory();
  function playerState() {
    const visible = new Set(trial.units.filter(visibleToFriendly).map(u=>u.id));
    const contacts=contactMemory.update(time,trial.units.filter(u=>!u.friendly&&visible.has(u.id)),p=>visibleToFriendly({...trial.units.find(u=>!u.friendly)!,...p,friendly:false}));
    const raw = trial.state();
    return {...raw, message, running, time, contacts, sandbags:sandbags.map(p=>({...p})),
      units: raw.units.filter(u=>visible.has(u.id)).map(u=>u.friendly ? u :
        {...u, path:[], guide:[], facing:undefined, aimAngle:undefined}),
      shots: shots.filter(s=>disclosedShots.has(s.id)&&visible.has(s.from)&&visible.has(s.to)),
  ...
  function command(
    ...
    if (action === "stop"||action==='hold') {trial.stop();for(const id of own)trial.units.find(u=>u.id===id)!.stance='hold';}
    if (action === "move" && x !== undefined && z !== undefined) {
      if(trial.order({ x, z },facing))for(const id of own){const u=trial.units.find(v=>v.id===id)!;u.stance='move';reactions.set(id,{anchor:{...(u.path.at(-1)??u)},next:time+3,health:u.health,target:null});}
    ...
    if (action === "attack") {
      ...
      if (!enemy || !visibleToFriendly(enemy)) { message = "Target is not currently visible."; return; }
      for (const id of own) {
      ...
      for(const id of own){const u=trial.units.find(v=>v.id===id)!;u.stance='attack';reactions.set(id,{anchor:{...(u.path.at(-1)??u)},next:time+3,health:u.health,target:String(enemy.id)});}
      trial.selectMany(own);
  ...
  function tick(dt: number) {
    ...
    while (accumulator >= 0.05) {
      ...
      time += 0.05;
      trial.tick(0.05);
      const crushed=sandbags.filter(p=>trial.units.some(u=>u.kind==="vehicle"&&u.health>0&&obstacleDistance(u,{...p,id:String(p.id),kind:"sandbag",width:4,depth:1})<.7));
      if(crushed.length){for(const p of crushed)sandbags.splice(sandbags.indexOf(p),1);syncSandbags();}
      for(const u of trial.units)if(u.kind==='vehicle'&&u.health>0){
        const current=u.turretAngle??u.angle,target=u.aimAngle??u.angle;
        const error=Math.atan2(Math.sin(target-current),Math.cos(target-current));
        u.turretAngle=current+Math.max(-.075,Math.min(.075,error));
      }
      ...
      for(const u of alive){
        if(u.kind!=='infantry'||u.stance==='hold'||u.path.length)continue;
        const s=squads.get(u.id)!;
        let memory=reactions.get(u.id);if(!memory){memory={anchor:{x:u.x,z:u.z},next:time+(u.id%7)*.3,health:u.health,target:null};reactions.set(u.id,memory);}
        if(time<memory.next)continue;
        memory.next=time+3.5+(u.id%5)*.3;
        const enemies=alive.filter(v=>v.friendly!==u.friendly&&Math.hypot(v.x-u.x,v.z-u.z)<30);
        const threat=enemies.find(v=>String(v.id)===s.target)??enemies.sort((a,b)=>Math.hypot(a.x-u.x,a.z-u.z)-Math.hypot(b.x-u.x,b.z-u.z))[0];
        if(!threat)continue;
      ...
      for (const u of alive) {
        ...
        if(u.kind==='vehicle'&&Math.abs(Math.atan2(Math.sin(u.aimAngle-(u.turretAngle??u.angle)),Math.cos(u.aimAngle-(u.turretAngle??u.angle))))>.12)continue;
        if(u.kind==='vehicle'){
        ...
        if(u.kind==='vehicle')nextShell.set(u.id,time+CITY_TANK_RELOAD_SECONDS);
        const impact = u.kind === "vehicle" ? cityShellImpact(target.x,target.z,volley.rawDamage>0 || (!u.moving && !target.moving && Math.hypot(target.x-u.x,target.z-u.z)<=12),sequence+1+u.id*7919) : target;
        const shot = emit({
          from: u.id,
          to: target.id,
          x: u.x,
        ...
        if (shot.shell)
          pending.push({ due: time + cityShellFlightSeconds(Math.hypot(shot.tx-shot.x,shot.tz-shot.z)), shot, damage: CITY_SHELL_DAMAGE });
        else
