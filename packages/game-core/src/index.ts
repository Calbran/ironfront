...
export interface Region {
  /** Saved geometry used for authoritative fire blocking; buildings exposure 0, low cover .5. */
  fireObstacles?: import("./squadFire.ts").FireObstacle[];
  id: number;
  name: string;
  x: number;
  y: number;
  ...
  purpose?:
    ...
    | "settled heartland"
    | "wilderness";
  terrainLayout?: import("./terrainLayout.ts").TerrainLayout;
...
export function createWorld(
  ...
  const w: World = {
    ...
    geography,
    nations,
    armies: nations.flatMap((n) =>
      (["line", "assault", "mobile"] as const).map((role, i) =>
...
export function path(w: World, from: number, to: number): number[] {
  if (
    !w.regions[from] ||
    !w.regions[to] ||
    w.regions[from].terrain === "mountains" ||
...
export const ARMY_ROLES = {
  line: {
    name: "Line army",
    description: "Infantry holds a sector; artillery wears down defenses.",
    infantry: 70,
    motorized: 0,
    artillery: 25,
    tanks: 5,
  ...
  assault: {
    name: "Assault army",
    description: "Combined arms for deliberate breakthroughs.",
    infantry: 35,
    motorized: 10,
    artillery: 25,
    tanks: 30,
  ...
  mobile: {
    name: "Mobile army",
    description:
      "Motorized infantry and armor exploit openings or reinforce a sector.",
    infantry: 10,
    motorized: 55,
    artillery: 10,
    tanks: 25,
...
export function makeArmy(
  ...
  return {
    ...
    motorized: composition.motorized,
    artillery: composition.artillery,
...
export function formationName(a: Army): string {
  return a.squadKind
    ? a.squadKind === "motorized"
      ? "Mobile infantry squad"
      : "Infantry squad"
    : ARMY_ROLES[a.role].name;
}
...
export function upgradeWorld(w: World): World {
  ...
  for (const a of w.armies) {
    ...
    Object.assign(a, makeArmy(a.id, a.owner, a.region, "line"), {
      ...
      air: old.air,
      infantry: old.infantry,
      artillery: old.artillery,
      tanks: old.tanks,
    });
  }
  if (w.winner === null)
  ...
  if (w.winner === null)
    report(
      w,
      "Front-line rules activated. Existing armies hold position; assault and mobile armies are ready. Review your sectors and objectives.",
    );
  return w;
}
export function friendlyPath(
  w: World,
  from: number,
  to: number,
  owner: number,
...
function restrictedPath(
  ...
  from: number,
  to: number,
  allowed: (r: Region) => boolean,
): number[] {
  if (!w.regions[from] || !w.regions[to] || !allowed(w.regions[from]))
    return [];
  const seen = new Set([from]),
    queue = [[from]];
  while (queue.length) {
...
export function offensivePath(w: World, a: Army, target: number): number[] {
  ...
  return restrictedPath(
    ...
    a.region,
    target,
    (r) => r.owner === a.owner || r.owner === enemy,
  );
}
...
export function supplyNetwork(w: World, owner: number): Set<number> {
  ...
  while (queue.length) {
    ...
    const remaining =
      r.building === "depot" && r.consolidation === 0 ? 4 : incoming;
...
export function supplied(w: World, a: Army): boolean {
  return supplyNetwork(w, a.owner).has(a.region);
}
...
export function coverage(w: World, a: Army): number[] {
  ...
  return a.sector.filter(
    (id) =>
      w.regions[id]?.owner === a.owner &&
...
export function command(w: World, owner: number, c: Command) {
  if (w.winner !== null)
    throw Error(
      "This campaign has ended. Create another campaign to play again.",
    );
  ...
  if (c.type === "squad-attack") {
    ...
    report(
      w,
      `${c.squads.length} squad(s): engaging designated target.`,
      "order",
      null,
      owner,
    );
  ...
  if (c.type === "garrison-squads") {
    const region = w.regions[c.region];
    const site = region?.features?.find(
      (f) => f.id === c.feature && f.kind === "settlement",
    );
    if (!site || settlementOwner(region, site) !== owner)
      throw Error("Choose a settlement you control.");
    orderSquads(w, owner, c.squads, "move", [{ x: site.x, y: site.y }]);
    for (const id of c.squads)
      w.tactics!.squads.find((s) => s.id === id)!.garrisonSite = {
        region: c.region,
        feature: c.feature,
      };
    report(
      w,
      `${c.squads.length} squad(s) ordered to garrison ${site.name}.`,
      "order",
      c.region,
      owner,
    );
  ...
  if (c.type === "squad-order") {
    ...
    report(
      w,
      `${c.squads.length} squad(s): ${c.mode === "hold" ? "holding position" : "following local waypoints"}.`,
      "order",
      null,
      owner,
    );
  ...
  if (c.type === "build") {
    const r = w.regions[c.region],
      b = BUILDINGS[c.building];
    if (!r || r.owner !== owner || r.terrain === "mountains")
      throw Error("Build in a region you own.");
    if (r.building || r.construction)
      throw Error(
        "This region already has a building or construction underway.",
      );
    if (n.industry < b.cost)
      throw Error(
        "Not enough industry. Let production replenish your reserves.",
      );
    n.industry -= b.cost;
    r.construction = { kind: c.building, remaining: b.hours };
    report(
      w,
      `${n.name} began ${b.name.toLowerCase()} at ${r.name}.`,
      "build",
      r.id,
      owner,
    );
  ...
  if (c.type === "cover") {
    ...
    command(w, owner, {
      type: "order",
      army: a.id,
      order: c.region === a.region ? "hold" : "redeploy",
      ...(c.region === a.region ? {} : { target: c.region }),
    });
    a.cover = proposed.cover;
    a.sector = [a.region];
    a.status = `Moving to cover: ${site.name}`;
    report(
      w,
      `${ARMY_ROLES[a.role].name} ordered to take cover at ${site.name}.`,
      "order",
      c.region,
      owner,
    );
  ...
  if (c.type === "policy") {
    if (
      c.fallback !== null &&
      !friendlyPath(w, a.region, c.fallback, owner).length
    )
      throw Error("Choose a fallback connected through friendly territory.");
    a.risk = c.risk;
    a.fallback = c.fallback;
    return;
  }
  if (c.type === "sector") {
    if (
      ...
      !c.regions.includes(a.region) ||
      c.regions.some(
        (id) =>
          w.regions[id]?.owner !== owner ||
          w.regions[id].terrain === "mountains" ||
          (id !== a.region && !w.regions[a.region].neighbors.includes(id)),
      )
    )
      throw Error(
        "A sector includes headquarters and up to two adjacent friendly regions.",
      );
    for (const s of w.tactics?.squads ?? [])
      if (s.army === a.id) {
        delete s.localOrder;
        delete s.independent;
    ...
    report(
      w,
      `${ARMY_ROLES[a.role].name} is deploying across ${a.sector.length} regions.`,
      "order",
      a.region,
      owner,
    );
  ...
  if (c.order === "advance" || c.order === "redeploy") {
    ...
    const route =
      c.order === "redeploy"
        ? friendlyPath(w, a.region, c.target, owner)
        : offensivePath(w, a, c.target);
    if (!route.length)
      throw Error(
        "No legal route. Redeploy through friendly land or select an objective across one opposing territory.",
      );
  ...
  a.status =
    c.order === "advance"
      ? "Advancing along planned corridor"
      : c.order === "redeploy"
        ? "Redeploying through friendly territory"
        : c.order === "reserve"
          ? "Watching sector for attacks"
          : c.order === "recover"
            ? "Recovering strength"
            : "Holding sector";
  report(
    w,
    `${ARMY_ROLES[a.role].name}: ${a.status.toLowerCase()}.`,
    "order",
    a.target ?? a.region,
    owner,
  );
...
export function withdraw(
  ...
  const route =
    a.fallback === null ? [] : friendlyPath(w, a.region, a.fallback, a.owner);
  ...
  moveArmy(a, next);
  stop(a, "recover", "Withdrew; recovering strength");
  ...
  if (a.fallback !== null && a.region !== a.fallback) {
    ...
    if (remaining.length > 1) {
      a.route = remaining.slice(1);
      a.target = a.fallback;
    }
  }
  report(
    w,
    `${ARMY_ROLES[a.role].name} withdrew to ${w.regions[next].name}.`,
    "combat",
    next,
    a.owner,
  );
...
export function advance(w: World, tacticalHours = 1) {
  ...
  for (const n of w.nations) {
    ...
    n.industry +=
      land.length * 0.25 +
      land.filter((r) => r.building === "factory").length * 2;
    n.fuel +=
      land.length * 0.12 +
      land.filter((r) => r.building === "refinery").length * 2;
  ...
  for (const r of w.regions) {
    ...
    if (r.construction && --r.construction.remaining <= 0) {
      r.building = r.construction.kind;
      r.construction = null;
      report(
        w,
        `${BUILDINGS[r.building].name} completed in ${r.name}.`,
        "build",
        r.id,
        r.owner,
      );
    }
    if (r.owner !== null && r.consolidation === 0)
      r.garrison = Math.min(32, r.garrison + 0.5);
  }
  const networks = w.nations.map((n) => supplyNetwork(w, n.id));
  for (const a of w.armies) {
    if (a.strength <= 0) continue;
    const direct =
      w.tactics?.squads.filter((s) => s.army === a.id && s.independent) ?? [];
    if (direct.length) {
      // Group logistics follow its actual squads; HQ no longer moves or captures for them.
      const n = w.nations[a.owner];
      const connected = direct.every(
        (s) => s.strength <= 0 || networks[a.owner].has(s.region),
      );
      a.supplies = connected
        ? Math.min(reserveCapacity(w, a), a.supplies + 2)
        : Math.max(0, a.supplies - 1);
      for (const s of direct) {
        if (s.strength <= 0) continue;
        if (!networks[a.owner].has(s.region) && a.supplies === 0)
          s.strength = Math.max(0, s.strength - 0.02 * s.capacity);
        else if (
          networks[a.owner].has(s.region) &&
          s.localOrder?.mode === "hold" &&
    ...
    const n = w.nations[a.owner],
      connected = networks[a.owner].has(a.region);
    ...
    if (n.bot && a.order === "hold" && a.strength > 60 && connected) {
      ...
      const front = w.regions[a.region].neighbors.filter(
        (id) =>
          w.regions[id].owner === a.owner &&
          w.regions[id].neighbors.some(
            (next) =>
              w.regions[next].owner !== a.owner &&
              w.regions[next].terrain !== "mountains",
          ),
      );
      if (a.role === "line" && front.length)
        a.sector = [a.region, ...front.slice(0, 2)];
    ...
    if (a.order === "advance" && a.strength < retreatThreshold(a))
      stop(a, "recover", "Offensive halted: recovering strength");
    if (
      (a.order === "hold" || a.order === "recover" || a.order === "reserve") &&
      !a.route.length
    ) {
      const amount = Math.max(
        0,
        Math.min(
          ...
          connected
            ? (a.order === "recover" ? 5 : 2) +
                (w.regions[a.region].building === "depot" ? 2 : 0)
            : 0,
        ),
      );
      a.strength += amount;
      n.manpower -= amount;
      n.industry -= amount / 2;
      if (n.bot && a.order === "recover" && a.strength >= 85)
        stop(a, "hold", "Ready for orders");
    }
    if (a.order === "hold" && a.deployment === 0)
      a.entrenchment = Math.min(6, a.entrenchment + 1);
  ...
  for (const a of w.armies.filter(
    (a) =>
      a.order === "reserve" &&
      !a.route.length &&
      a.strength >= retreatThreshold(a),
  )) {
    const threatened = a.sector.find((id) =>
      w.armies.some(
        (enemy) =>
          enemy.owner !== a.owner &&
          enemy.order === "advance" &&
    ...
    if (route.length > 1) {
      a.route = route.slice(1);
      a.target = threatened;
      a.status = "Reserve responding to attack";
    } else if (route.length === 1)
      stop(a, "hold", "Reserve holding threatened region");
  }
  ...
  for (const a of w.armies) {
    ...
    const target = a.route[0],
      next = w.regions[target],
      local = w.regions[a.region];
    if (
      !local.neighbors.includes(target) ||
      !next ||
      next.terrain === "mountains" ||
      (a.order !== "advance" && next.owner !== a.owner) ||
      (a.order === "advance" &&
        next.owner !== a.owner &&
        next.owner !== a.campaignOwner)
    ) {
      stop(a, "hold", "Route blocked: choose a new objective");
      report(
        w,
        `${ARMY_ROLES[a.role].name} halted: its planned route is blocked.`,
        "order",
        a.region,
        a.owner,
      );
    ...
    if (a.order === "advance" && local.consolidation > 0) {
      a.status = `Consolidating ${local.name}: ${local.consolidation}h`;
      continue;
    }
    if (a.order === "advance" && a.supplies < 3) {
      a.status = "Offensive paused: waiting for supply";
      continue;
    }
  ...
  for (const x of intents)
    for (const y of intents)
      if (
        x.army.owner !== y.army.owner &&
        x.from === y.target &&
        y.from === x.target
      ) {
        blocked.add(x.army.id);
        blocked.add(y.army.id);
      }
  for (const x of intents) {
    const other = intents.find(
      (y) =>
        x.army.owner !== y.army.owner &&
        x.from === y.target &&
        y.from === x.target,
    );
    if (other && x.army.id < other.army.id)
      beginEngagement(w, x.army, x.target);
  }
  intents.sort(
    (x, y) =>
      ((x.army.owner + w.hour) % w.nations.length) -
        ((y.army.owner + w.hour) % w.nations.length) || x.army.id - y.army.id,
  );
  for (const { army: a, from, target } of intents) {
    if (
      ...
      a.region !== from ||
      a.route[0] !== target ||
      w.regions[from].owner !== a.owner
    )
      continue;
    const r = w.regions[target],
      n = w.nations[a.owner];
    if (r.owner === a.owner) {
      moveArmy(a, target);
      continue;
    }
    if (a.order !== "advance" || r.owner !== a.campaignOwner) {
      stop(a, "hold", "Route blocked: ownership changed");
      continue;
    }
  ...
  for (const a of w.armies) {
    if (w.tactics?.squads.some((s) => s.army === a.id && s.independent))
      continue;
    if (a.target === a.region && !a.route.length) {
      const cover = a.cover;
      stop(a, a.order === "recover" ? "recover" : "hold", "Objective reached");
      if (cover?.region === a.region) a.cover = cover;
    }
    if (
      a.strength > 0 &&
    ...
    if (a.strength <= 0)
      report(
        w,
        `${ARMY_ROLES[a.role].name} of ${w.nations[a.owner].name} was destroyed or surrendered.`,
        "combat",
        a.region,
        a.owner,
      );
