...
w.tactics!.squads.push({
  ...ownSquad,
  id: `garrison-${ownSquad.region}`,
  army: null,
  owner: null,
  independent: false,
  kind: "garrison",
  x: ownSquad.x + 65,
  previousX: ownSquad.x + 65,
  unitCount: 3,
});
...
try {
  for (const width of [1440, 390]) {
    ...
    store.mutate(w.id, (saved) => {
      Object.assign(saved, structuredClone(w));
    });
    ...
    for (const detailed of [false, true]) {
      ...
      await page.mouse.click(px - 10, py);
      ...
      assert.equal(
        await page
          .getByLabel("Selected squad health", { exact: true })
          .getAttribute("value"),
        "100",
      );
      assert.equal(
        await page
          .getByLabel("Selected squad morale", { exact: true })
          .getAttribute("value"),
        "100",
      );
      if (detailed) {
        const tx = px + 65 * scale;
        await page.mouse.move(tx, py);
        await page.waitForTimeout(100);
        const cursor = await canvas
          .locator("canvas")
          .evaluate((el) => getComputedStyle(el).cursor);
        assert(cursor.includes("circle"), "Attack crosshair on defender hover");
        await page.screenshot({path: `.impeccable/review/attack-hover-${width}.png`});
        const response = page.waitForResponse((res) =>
          res.url().endsWith("/api/command"),
        );
        await page.mouse.click(tx, py, { button: "right" });
        assert.equal((await response).status(), 200);
        assert.equal(
          store.get(w.id)!.tactics!.squads.find((s) => s.id === ownSquad.id)!
            .localOrder!.attackTarget,
          `garrison-${ownSquad.region}`,
        );
        store.mutate(w.id, (saved) => {
          saved.regions[ownSquad.region].owner = null;
          saved.regions[ownSquad.region].garrison = 100;
          advanceTactics(saved, 0.01);
        });
        await page.waitForTimeout(3300);
      }
      await page.screenshot({
        path: `.impeccable/review/attack-${width}-${detailed}.png`,
      });
      await page
        .getByRole("button", { name: "Close information panel", exact: true })
        .click();
    ...
    await page.close();
    console.log(
      `${width}px: attack cursor, accepted target order, target reticle and server-confirmed firing; no errors/overflow`,
    );
  }
} finally {
  await browser.close();
