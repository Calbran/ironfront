...
function harness(perspective = false) {
  ...
  const camera = perspective
    ? new T.PerspectiveCamera(45, 800 / 600, 0.1, 10000)
    : new T.OrthographicCamera(-40, 40, 30, -30, 0.1, 1000);
...
test("perspective wheel zoom remains responsive and keeps its ground anchor fixed", () => {
  const h = harness(true);
  try {
    const ndc = new T.Vector2(0.45, -0.15),
      ray = new T.Raycaster();
    ray.setFromCamera(ndc, h.camera);
    const anchor = ray.ray.intersectPlane(
      ...
      before = h.camera.position.distanceTo(h.target);
    h.send("wheel", { clientX: 580, clientY: 345, deltaY: 300 });
    const after = anchor.clone().project(h.camera);
    assert.ok(h.camera.position.distanceTo(h.target) < before * 0.6);
    assert.ok(Math.hypot(after.x - ndc.x, after.y - ndc.y) < 1e-7);
  } finally {
    h.close();
  }
});
...
test("right-click orders remain separate from camera navigation and handlers dispose", () => {
  ...
  try {
    ...
    h.send("pointerdown", { button: 2, shiftKey: true });
    h.send("pointerup", { button: 2, shiftKey: true });
    assert.equal(h.orders[1][2], true);
  } finally {
    h.close();
  }
